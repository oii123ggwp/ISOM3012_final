...
if (isset($_GET['product_id']) && isset($_GET['product_type'])) {
   record_click($_GET['product_id']);
   record_click($_GET['product_type']);
   header("Location: search_detail.php?product_type=" . $_GET['product_type'] . "&product_id=" . $_GET['product_id']);
}
      ...
      if (isset($_POST['cartoon'])) //if user click cartoon submit button
      {
         ...
         if ($result = mysqli_query($link, $selected_product_sql)) { //checking error
            while ($row = mysqli_fetch_assoc($result)) { //show the result on the page
               echo "<div class='row0'>";
               echo "<a href='search_product.php?product_type=" . $row['Type'] . "&product_id=" . $row['product_id'] . "'>";
